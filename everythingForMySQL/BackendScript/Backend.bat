...
set BACKEND_DIR=%~dp0backend-Node-mysql
...
echo //route for tablenames>> "%ROUTE_FILE%"
echo router.get('/tablenames', (req, res) =^> {>> "%ROUTE_FILE%"
echo     getTableNames((err, data) =^> {>> "%ROUTE_FILE%"
echo       if (err) return res.status(500).send('Erreur serveur');>> "%ROUTE_FILE%"
