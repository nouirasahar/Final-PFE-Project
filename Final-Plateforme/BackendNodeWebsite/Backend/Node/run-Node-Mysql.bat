...
:: Define file paths for mysqlService.js, dbConnection, apiRoutes.js and index.js
...
echo     res.send('Welcome to your API!'); >> "%ROUTE_FILE%"
echo   }); >> "%ROUTE_FILE%">> "%ROUTE_FILE%"

echo //  Route to fetch all  table names >> "%ROUTE_FILE%"
...
echo     // fetch ONLY table names, ignoring the  "Tables_in_ma_base" >> "%ROUTE_FILE%"
echo     const tableNames = results.map(row =^> Object.values(row)[0]);>> "%ROUTE_FILE%"

echo     // JSON return for tablenames >> "%ROUTE_FILE%"
...
echo       if (err) { >> "%ROUTE_FILE%"
echo         return res.status(500).send('server error'); >> "%ROUTE_FILE%"
echo       } >> "%ROUTE_FILE%"

echo       const tableNames = tables.map(table =^> Object.values(table)[0]); >> "%ROUTE_FILE%"

echo       // For each table, we get the data  >> "%ROUTE_FILE%"
...
echo             if (err) { >> "%ROUTE_FILE%"
echo               reject(`Error with the table  ${table}: ${err}`); >> "%ROUTE_FILE%"
...
echo       // wait for the tables data >> "%ROUTE_FILE%"
echo       Promise.all(promises) >> "%ROUTE_FILE%"
echo         .then(results =^> { >> "%ROUTE_FILE%"
echo           // Create an objet with the data of all tables>> "%ROUTE_FILE%"
...
echo           // Return tables data >> "%ROUTE_FILE%"
echo           res.json(allTablesData); >> "%ROUTE_FILE%"
echo         }) >> "%ROUTE_FILE%"
echo         .catch(err =^> { >> "%ROUTE_FILE%"
echo           res.status(500).send(`Error while fetching data from tables: ${err}`); >> "%ROUTE_FILE%"
echo   }); >> "%ROUTE_FILE%"
echo     }); >> "%ROUTE_FILE%"
echo   }); >> "%ROUTE_FILE%"
...
echo  // Checks that there is indeed data to update >>"%ROUTE_FILE%"
echo  if ^(^!updateData ^|^| Object.keys^(updateData^).length === 0^) { >>"%ROUTE_FILE%"
echo   return res.status(400).json({ error: 'No data to updater' }); >>"%ROUTE_FILE%"
echo  } >>"%ROUTE_FILE%"
...
echo  // Build the SQL query >>"%ROUTE_FILE%"
echo  const sql = `UPDATE \`${table}\` SET ${columns} WHERE id = ?`; >>"%ROUTE_FILE%"
echo  const db = require('../Controllers/dbConnection'); >>"%ROUTE_FILE%"
echo  db.query(sql, [...values, id], (err, result) =^> { >>"%ROUTE_FILE%"
echo   if (err) { >>"%ROUTE_FILE%"
echo    return res.status(500).json({ message: 'Error during update', error: err }); >>"%ROUTE_FILE%"
echo   } >>"%ROUTE_FILE%"
echo   if (result.affectedRows === 0) { >>"%ROUTE_FILE%"
echo    return res.status(500).json({ message: 'Error during update', error: err }); >>"%ROUTE_FILE%"
echo   } >>"%ROUTE_FILE%"
echo   if (result.affectedRows === 0) { >>"%ROUTE_FILE%"
echo    return res.status(404).json({ message: 'No record found with this ID' }); >>"%ROUTE_FILE%"
echo   } >>"%ROUTE_FILE%"
echo   res.status(200).json({ message: 'Update successful', result }); >>"%ROUTE_FILE%"
echo  }); >>"%ROUTE_FILE%"
echo }); >>"%ROUTE_FILE%"

echo // Route to retrieve a single item from a table by ID >>"%ROUTE_FILE%"
...
echo    if ^(err^) { >>"%ROUTE_FILE%"
echo      return res.status^(500^).json^({ message: 'Server error', error: err }^); >>"%ROUTE_FILE%"
echo    } >>"%ROUTE_FILE%"

echo    if ^(results.length === 0^) { >>"%ROUTE_FILE%"
echo      return res.status^(404^).json^({ message: 'No itme with this ID' }^); >>"%ROUTE_FILE%"
echo    } >>"%ROUTE_FILE%"
