...
echo }>>"%SERVICE_FILE%"
echo //function to update an item>>"%SERVICE_FILE%"
...
echo     console.log(`→ Deleting from ${req.params.table}, id: ${req.params.id}`);>> "%ROUTE_FILE%"
echo     deleteItemById(req.params.table, req.params.id)>> "%ROUTE_FILE%"
echo       .then(result=^> {>> "%ROUTE_FILE%"
echo         console.log('→ Item deleted');>> "%ROUTE_FILE%"
echo         res.json(r);>> "%ROUTE_FILE%"
echo       })>> "%ROUTE_FILE%"
echo       .catch(() =^> res.sendStatus(500));>> "%ROUTE_FILE%"
echo   });>> "%ROUTE_FILE%"
